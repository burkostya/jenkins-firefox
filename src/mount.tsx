...
export function mount(location:JenkinsLocation = parseLocation(window.location.href)){
  ...
  function close(){if(closed)return;closed=true;showClassic(true);root.unmount();host.remove();window.removeEventListener('pagehide',close);}
  window.addEventListener('pagehide',close,{once:true});
  root.render(<ErrorBoundary onClose={close}><App classicLabel={original?.id==='nodeGraph'?'Original Pipeline Steps':'Original Stage View'} {...{location,portal,host}} onClassic={showClassic} onClose={close}/></ErrorBoundary>);
}
