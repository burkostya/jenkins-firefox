...
with sync_playwright() as p:
 ...
 expect(server.get_by_role('button',name='Dark',exact=True)).to_have_count(0)
 expect(server.get_by_role('button',name='Light',exact=True)).to_have_count(0)
 server.emulate_media(color_scheme='dark')
 expect(server.locator('#pipeline-graph-local-extension')).to_have_attribute('data-theme','dark')
 server.emulate_media(color_scheme='light')
 expect(server.locator('#pipeline-graph-local-extension')).to_have_attribute('data-theme','light')
 ok('theme follows browser prefers-color-scheme changes without a manual override')
