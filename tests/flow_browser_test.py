...
with sync_playwright() as p:
 ...
 def page_for(mode='html',flow=FLOW,runs=RUNS,steps_page=False):
  ...
  return page
 page=page_for()
 expect(page.locator('.pgvx-notice')).to_contain_text('Source: Pipeline Steps HTML')
 ...
 expect(native.locator('.pgvx-notice')).to_contain_text('Jenkins execution tree')
 ...
 expect(fallback.locator('.pgvx-notice')).to_contain_text('Pipeline Steps HTML')
 expect(fallback.locator('.pgvx-warning')).to_contain_text('HTTP 403')
 ...
 expect(wrong.locator('.pgvx-warning')).to_contain_text('selected build')
 ...
 expect(broken.locator('.pgvx-warning')).to_contain_text('unsupported indentation')
 ...
 expect(login.locator('.pgvx-warning')).to_contain_text('expected one #nodeGraph table')
